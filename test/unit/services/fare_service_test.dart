...
void main() {
  group('FareService', () {
    ...
    group(
      'estimateSharedDistanceFare - Distance-based proportional pricing',
      () {
        test('splits fare proportionally based on distance', () async {
          ...
          expect(
            result.passengerFares[0].total,
            greaterThan(result.passengerFares[1].total),
          );

          // Verify passenger 1 pays roughly 2x what passenger 2 pays
          final ratio =
              result.passengerFares[0].total / result.passengerFares[1].total;
        ...
        test('handles multiple passengers with varying distances', () async {
          ...
          final p3 = result.passengerFares.firstWhere(
            (p) => p.passengerId == 'passenger_3',
          );
          final p2 = result.passengerFares.firstWhere(
            (p) => p.passengerId == 'passenger_2',
          );
          final p1 = result.passengerFares.firstWhere(
            (p) => p.passengerId == 'passenger_1',
          );
        ...
        test(
          'SharedFareBreakdown.toMap contains all required fields',
          () async {
            final pickup = LatLng(7.0711, 125.6088);
            final destination = LatLng(7.0652, 125.6126);
        ...
        test('calculates correct total driver take', () async {
          ...
          expect(
            result.totalDriverTake,
            closeTo(result.totalFare - result.totalPlatformFee, 0.01),
          );
        });
      },
    );
