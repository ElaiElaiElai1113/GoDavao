...
class PassengerRideService {
  ...
  Future<Map<String, dynamic>> fetchMatchFacts(String rideId) async {
    try {
      ...
      final seats = ((row['seats_allocated'] as num?)?.toInt() ?? 1).clamp(
        1,
        6,
      );

      return {'matchId': row['id']?.toString(), 'seatsBilled': seats};
    } catch (e) {
    ...
    void handler(PostgresChangeEvent event) {
      channel.onPostgresChanges(
        ...
        callback: (payload) {
          ...
          final parsed =
              n?.toDouble() ?? double.tryParse(rec?['value']?.toString() ?? '');
  ...
  Map<String, dynamic>? extractDriverInfo(Map<String, dynamic>? ride) {
    ...
    if (users == null) {
      return {'id': driverMap['driver_id']?.toString(), 'name': 'Driver'};
    }
